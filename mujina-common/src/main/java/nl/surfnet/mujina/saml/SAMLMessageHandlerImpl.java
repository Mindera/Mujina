...
public class SAMLMessageHandlerImpl implements SAMLMessageHandler {
  ...
  private static String POST_UNSIGNED_TEMPLATE = "/templates/saml2-post-simplesign-binding.vm";
  private static String POST_SIMPLE_SIGNED_TEMPLATE = "/templates/saml2-post-simplesign-binding.vm";
  private static String POST_TEMPLATE = "/templates/saml2-post-binding.vm";

  private VelocityEngine velocityEngine;

  private final SAMLMessageDecoder decoder;
  private final SecurityPolicyResolver resolver;
  private final String acsBindingURI;

  private String entityId;
  private boolean needsSigning;

  public SAMLMessageHandlerImpl(SAMLMessageDecoder decoder, SecurityPolicyResolver resolver, String acsBindingURI) {
    super();
    this.decoder = decoder;
    this.resolver = resolver;
    this.acsBindingURI = acsBindingURI;
  }
  ...
  public void sendSAMLMessage(SignableSAMLObject samlMessage, Endpoint endpoint, HttpServletResponse response, String relayState, Credential signingCredential) throws MessageEncodingException {
    ...
    BaseSAML2MessageEncoder encoder = getMessageEnconder(velocityEngine, isNeedsSigning());
  ...
  private BaseSAML2MessageEncoder getMessageEnconder(VelocityEngine engine, boolean signXMLProtocolMessage) {
    if (SAMLConstants.SAML2_POST_SIMPLE_SIGN_BINDING_URI.equals(this.acsBindingURI)) {
      return new HTTPPostSimpleSignEncoder(engine, getTemplate(), signXMLProtocolMessage);
    } else {
      return new HTTPPostEncoder(engine, getTemplate());
    }
  }

  private String getTemplate() {
    if (SAMLConstants.SAML2_POST_SIMPLE_SIGN_BINDING_URI.equals(this.acsBindingURI)) {
      return this.isNeedsSigning() ? POST_SIMPLE_SIGNED_TEMPLATE : POST_UNSIGNED_TEMPLATE;
    } else {
